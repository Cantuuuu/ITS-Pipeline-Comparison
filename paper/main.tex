...
%% ----------------------------------------------------------------
\begin{abstract}
Automated individual tree segmentation (ITS) in high-density UAV laser scanning (ULS) point clouds is critical for scalable forest inventories, yet the relative contribution of semantic preprocessing and instance segmentation to end-to-end ITS error remains poorly isolated experimentally. We present a controlled factorial comparison ($3 \times 2$ design) of three preprocessing strategies --- no preprocessing, Random Forest (RF) on geometric features, and PointNet++ MSG --- combined with two variants of an interpretable volumetric Watershed 3D segmenter (classic CHM seeding and a novel density-based seeding), calibrated independently per pipeline and evaluated on the FOR-instance~V1 benchmark across five forest types. Despite near-perfect semantic quality (tree~IoU~$>$~0.97 for both classifiers), instance F1 peaks at 0.209,revealing a strong dissociation between semantic segmentation quality and instance-level performance, where the dominant source of error emerges from instance separation rather than semantic preprocessing. Within the evaluated configurations,RF with geometric features outperforms PointNet++ MSG across all pipeline combinations without requiring a GPU, and density-based seeding consistently improves over CHM seeding (+47\% for RF, +74\% for PointNet++), particularly in dense uniform-height canopies where CHM is nearly flat.These findings are specifically associated with dense ULS forest conditions, where extreme point density and canopy continuity amplify the geometric complexity of individual tree separation.
...
Los inventarios forestales automatizados a nivel de \'arbol individual son esenciales para la estimaci\'on de carbono almacenado, el monitoreo de la biodiversidad y la silvicultura de precisi\'on. El escaneo l\'aser UAV (ULS) proporciona densidades de puntos entre uno y dos \'ordenes de magnitud superiores a las del ALS convencional ($\sim$7{,}000~pts/m$^2$ vs.\ $\sim$5~pts/m$^2$), lo que permite caracterizar \'arboles individuales, incluidas sus copas, troncos, ramas y sotobosque. Sin embargo, la segmentaci\'on autom\'atica de \'arboles individuales (ITS) en datos ULS sigue siendo un problema abierto debido al ruido en los modelos de altura, la ambig\"uedad entre copa y sotobosque, y la variabilidad estructural entre tipos de bosque.A diferencia del ALS convencional, los datos ULS presentan densidades extremadamente altas y una continuidad vertical mucho más pronunciada dentro del dosel forestal. Esta diferencia modifica sustancialmente la representación geométrica de las copas y altera el comportamiento de los algoritmos de segmentación de instancias, particularmente en bosques densos donde múltiples árboles comparten regiones espaciales continuas.



Los pipelines de ITS suelen operar en dos etapas encadenadas: un clasificador sem\'antico que a\'isla los puntos de \'arbol respecto al suelo, sotobosque y ruido, seguido de un segmentador de instancias que asigna a cada punto de \'arbol un identificador \'unico. Los m\'etodos de aprendizaje profundo, como PointNet++~\cite{qi2017}, han generado altas expectativas respecto a la calidad sem\'antica; sin embargo, su ventaja pr\'actica frente a clasificadores cl\'asicos como Random Forest en el contexto espec\'ifico del preprocesado para ITS a\'un no est\'a clara. Al mismo tiempo, los m\'etodos volum\'etricos de Watershed~3D han evolucionado m\'as all\'a del enfoque cl\'asico 2D basado en CHM~\cite{dalponte2016}, permitiendo explotar directamente la densidad vertical de puntos. A pesar de estos avances, el impacto relativo de cada etapa sobre el rendimiento extremo a extremo rara vez se ha aislado experimentalmente~\cite{qi2017,breiman2001,puliti2023}, la mayoría de los trabajos recientes en ITS se enfocan principalmente en incrementar el desempeño global mediante arquitecturas end-to-end cada vez más complejas. Sin embargo, al acoplar estrechamente la clasificación semántica y la segmentación de instancias dentro de modelos tipo black-box, resulta difícil identificar cuál de las etapas domina realmente el error final del pipeline.
...
Los enfoques recientes de segmentación end-to-end han demostrado mejoras importantes en benchmarks forestales mediante arquitecturas altamente acopladas capaces de integrar representación semántica y agrupamiento de instancias dentro de un mismo modelo. Métodos recientes como TreeLearn~\cite{henrich2024}, SegmentAnyTree~\cite{wielgosz2024} y ForestFormer3D~\cite{xiang2025} representan esta tendencia hacia modelos de segmentación altamente integrados y orientados a maximizar desempeño absoluto.

Sin embargo, esta integración también dificulta aislar experimentalmente qué componente del pipeline domina realmente el error final de segmentación. En consecuencia, aunque dichos métodos pueden alcanzar mejores métricas globales, ofrecen menor interpretabilidad respecto a la contribución relativa del preprocesado semántico y de los mecanismos geométricos de separación de instancias.
...
En lugar de proponer una nueva arquitectura ITS end-to-end, este trabajo adopta un diseño factorial interpretable orientado a aislar experimentalmente el efecto del preprocesado semántico y de las estrategias de separación de instancias sobre el desempeño final del pipeline. Watershed 3D fue seleccionado deliberadamente como segmentador base debido a su interpretabilidad y a su dependencia explícita de la generación geométrica de semillas, permitiendo observar cómo los errores semánticos se propagan hacia la segmentación de instancias bajo condiciones controladas. El enfoque experimental adoptado busca caracterizar específicamente el comportamiento del pipeline ITS bajo escenarios ULS densos, donde la continuidad vertical y el traslape de copas generan condiciones considerablemente más complejas para la separación geométrica de instancias que en adquisiciones ALS convencionales.

A diferencia de arquitecturas end-to-end modernas, el pipeline adoptado en este trabajo prioriza interpretabilidad experimental sobre maximización directa de desempeño absoluto. Esta decisión permite desacoplar explícitamente los efectos del preprocesado semántico y de la segmentación geométrica, facilitando el análisis del origen estructural del error dentro del ITS bajo ambientes ULS densos.
...
Todos los experimentos fueron implementados en Python utilizando Open3D, Scikit-learn y PyTorch. El entrenamiento de PointNet++ se realizó utilizando una GPU NVIDIA RTX~3060 con 12\,GB de VRAM, mientras que los pipelines basados en Random Forest fueron ejecutados completamente en CPU.

El entrenamiento de RF requirió aproximadamente 16 minutos por ejecución, mientras que PointNet++ requirió aproximadamente 2.5 horas de entrenamiento para 100 épocas bajo la configuración evaluada. Los experimentos utilizaron semillas fijas para train/validation split y calibración de hiperparámetros.

El código fuente completo, scripts de reproducción y configuraciones experimentales están disponibles públicamente en:
\url{https://github.com/Cantuuuu/ITS-Pipeline-Comparison}
...
La Tabla~\ref{tab:results-agg} presenta los resultados sobre el conjunto de test. El Flujo~D (RF + Density) alcanza el mejor desempeño agregado de segmentación de instancias (F1\,=\,0.209), mientras que el seeding por densidad mejora consistentemente ambos clasificadores respecto al CHM tradicional.

\begin{table}[H]
\centering
\caption{Resultados agregados (test, IoU~3D~$\geq 0.5$, media $\pm$ SD sobre $n=11$~plots).}\label{tab:results-agg}
\resizebox{\textwidth}{!}{%
...
B --- Random Forest      & CHM     & 0.985 & 0.990 & 0.996 & $0.394 \pm 0.402$ & $0.110 \pm 0.118$ & $0.142 \pm 0.226$ & $0.110 \pm 0.118$ \\
C --- PointNet++ MSG     & CHM     & 0.974 & 0.982 & 0.994 & $0.172 \pm 0.259$ & $0.060 \pm 0.079$ & $0.082 \pm 0.162$ & $0.060 \pm 0.079$ \\
D --- Random Forest      & Density & 0.985 & 0.990 & 0.996 & $0.418 \pm 0.291$ & $0.150 \pm 0.097$ & $\mathbf{0.209 \pm 0.161}$ & $0.150 \pm 0.097$ \\
E --- PointNet++ MSG     & Density & 0.974 & 0.982 & 0.994 & $0.209 \pm 0.148$ & $0.118 \pm 0.076$ & $0.143 \pm 0.109$ & $0.118 \pm 0.076$ \\
\bottomrule
\multicolumn{9}{l}{\footnotesize $^\dagger$SD $>$ media en algunos flujos refleja distribuciones de F1 fuertemente asim\'etricas con acumulaci\'on en cero (plots donde el segmentador falla completamente); v\'ease mediana en resultados complementarios.}
\end{tabular}%
}
\end{table}
...
\paragraph{An\'alisis de significancia estad\'istica.}
Para validar formalmente las diferencias observadas, se emplea un test de permutaci\'on exacto pareado (bilateral, $2^{11} = 2\,048$ permutaciones) sobre las m\'etricas de instancia F1 por plot ($n=11$), complementado con intervalos de confianza bootstrap BCa al 95\% ($10\,000$ remuestreos, semilla fija). Se elige el test de permutaci\'on sobre Wilcoxon signed-rank o $t$-Student porque: (i)~no requiere supuestos distribucionales, lo cual es cr\'itico dado que la distribuci\'on de F1 por plot es fuertemente asim\'etrica con acumulaci\'on en cero; (ii)~con $n=11$ se puede calcular la distribuci\'on nula exacta; y (iii)~a diferencia de Wilcoxon, preserva la magnitud de las diferencias, no s\'olo sus rangos. La Tabla~\ref{tab:significance} resume los resultados. La superioridad de RF sobre PointNet++ es estad\'isticamente significativa tanto con seeding CHM ($p=0.016$) como con Density ($p=0.020$), y la mejora del pipeline completo RF+Density respecto al baseline tambi\'en lo es ($p=0.016$). El density seeding alcanza significancia marginal ($p=0.096$ para RF, $p=0.066$ para PointNet++): si bien la significancia estricta a $\alpha=0.05$ est\'a limitada por el tama\~no muestral del benchmark, la distribuci\'on exacta de permutaciones, combinada con los intervalos BCa, confirma una ventaja direccional robusta --- particularmente en canopias densas de altura uniforme --- con mejora en 8/11~plots para RF y 8/11 para PointNet++. La reducci\'on de varianza del density seeding (SD de F1: $0.226 \rightarrow 0.161$ para RF) indica adicionalmente mayor robustez entre tipos de bosque.
...
La dificultad observada no debe interpretarse únicamente como una limitación aislada del algoritmo Watershed~3D, sino como una manifestación de un problema geométrico más general en ambientes ULS densos. En canopias altamente continuas, la separación de instancias depende de la existencia de discontinuidades espaciales, mínimos locales o contrastes de densidad que permitan inferir fronteras entre árboles adyacentes. Cuando dichas discontinuidades son débiles o inexistentes, múltiples árboles pueden compartir una misma masa volumétrica de puntos, reduciendo la separabilidad geométrica incluso antes de aplicar un algoritmo específico de segmentación. Por esta razón, aunque segmentadores más recientes podrían mejorar el desempeño absoluto, los resultados sugieren que cualquier método basado principalmente en señales geométricas locales enfrentará dificultades similares cuando las copas se superponen, las alturas son uniformes y la densidad de puntos genera continuidad volumétrica entre individuos. En este sentido, Watershed~3D funciona aquí como un instrumento interpretable para exponer una limitación estructural de la separación de instancias en ULS denso, más que como una afirmación de optimalidad algorítmica.

La dominancia del error de instancia observada en este trabajo probablemente se encuentra amplificada por las propiedades propias de adquisiciones ULS densas, donde la continuidad espacial entre copas y la superposición vertical son considerablemente mayores que en escenarios ALS tradicionales. En adquisiciones ALS de menor densidad, donde las copas suelen representarse mediante estructuras más dispersas y con menor continuidad vertical, la relación relativa entre error semántico y error de instancia podría diferir sustancialmente.

Aunque métodos recientes como TreeLearn~\cite{henrich2024}, SegmentAnyTree~\cite{wielgosz2024} y ForestFormer3D~\cite{xiang2025} probablemente podrían alcanzar mejores métricas absolutas de segmentación, el objetivo de este trabajo no consiste en competir directamente contra arquitecturas estado-del-arte, sino en analizar experimentalmente la relación entre calidad semántica y desempeño de separación de instancias dentro de un pipeline interpretable y controlado.

En este contexto, el uso de Watershed~3D resulta útil precisamente porque expone explícitamente cómo los errores geométricos emergen y se propagan a través del proceso de segmentación. Esta interpretabilidad sería considerablemente más difícil de obtener utilizando arquitecturas completamente acopladas de tipo black-box.
...
Aunque RF supera consistentemente a PointNet++ dentro de las configuraciones evaluadas, estos resultados no deben interpretarse como una superioridad general de clasificadores clásicos sobre arquitecturas profundas para segmentación semántica forestal. PointNet++ fue evaluado bajo una configuración limitada de hiperparámetros y entrenamiento, por lo que configuraciones más extensivas o datasets de entrenamiento mayores podrían modificar parcialmente esta relación.

Sin embargo, incluso bajo esta limitación, los resultados siguen mostrando que mejoras semánticas moderadas no necesariamente se traducen en mejoras proporcionales a nivel de instancia, reforzando la hipótesis central de desacoplamiento entre calidad semántica y separación geométrica de individuos.

\subsection{Limitaciones}

\paragraph{Tama\~no y balance del conjunto de evaluaci\'on.}
El conjunto de test comprende 11~plots y 323~\'arboles GT, con una concentraci\'on significativa en un solo tipo de bosque: NIBIO (boreal con\'ifero) aporta 6 de 11 plots (${\sim}50\%$) y 161 de 323 \'arboles (${\sim}50\%$), mientras que CULS, TUWIEN y RMIT contribuyen un \'unico plot cada uno. Esta distribuci\'on introduce dos efectos cuantificables: (i)~alta varianza en las estimaciones por sitio --- los intervalos de confianza bootstrap al 95\% de F1 para el mejor flujo (D) abarcan $[0.128, 0.314]$, reflejando que sitios individuales como CULS (F1\,=\,0.629) o NIBIO\_5 (F1\,=\,0.000) dominan la dispersi\'on; y (ii)~posible sesgo en las m\'etricas agregadas hacia el comportamiento en bosque boreal denso, que es el escenario m\'as desafiante para todos los m\'etodos.

\paragraph{Poder estad\'istico.}
Con $n=11$ plots, incluso el test de permutaci\'on exacto --- que maximiza el poder estad\'istico al preservar la magnitud de las diferencias y calcular la distribuci\'on nula exacta ($2^{11}$ permutaciones) --- alcanza s\'olo significancia marginal para el efecto del density seeding ($p=0.096$ para RF, $p=0.066$ para PointNet++). Esto no invalida el hallazgo: el efecto es consistente en 8 de 11~plots y los intervalos BCa para PointNet++ excluyen el cero. Sin embargo, la validaci\'on en FOR-instanceV2~\cite{xiang2025}, que ofrece mayor n\'umero de plots y diversidad, es necesaria para confirmar la significancia estricta de este efecto. Las comparaciones entre preprocesadores s\'i alcanzan significancia plena (RF vs.\ PointNet++, $p=0.016$; baseline vs.\ RF+Density, $p=0.016$) y son robustas incluso con esta muestra.
...
PointNet++ se eval\'ua con una \'unica configuraci\'on de hiperpar\'ametros; una b\'usqueda exhaustiva podr\'ia mejorar su rendimiento. Por esta raz\'on, las comparaciones entre RF y PointNet++ deben interpretarse principalmente como observaciones dentro del pipeline experimental evaluado y no como una comparaci\'on definitiva entre paradigmas cl\'asicos y deep learning para segmentaci\'on forestal. Las conclusiones sobre el cuello de botella son espec\'ificas al Watershed~3D volum\'etrico; otros segmentadores (TreeLearn~\cite{henrich2024}, SegmentAnyTree~\cite{wielgosz2024}) podr\'ian mostrar una relaci\'on diferente. No obstante, los patrones de error observados --- especialmente la fusi\'on de copas adyacentes en canopias continuas --- sugieren una dificultad geom\'etrica m\'as amplia para la separaci\'on de instancias en ULS denso. Los resultados son espec\'ificos a datos ULS ($454$--$10\,156$~pts/m$^2$); las conclusiones presentadas deben interpretarse espec\'ificamente dentro del contexto de nubes ULS densas y no como una caracterizaci\'on universal del comportamiento de pipelines ITS sobre todas las modalidades LiDAR. Asimismo, este trabajo no busca establecer una comparaci\'on cuantitativa exhaustiva frente a arquitecturas estado-del-arte recientes, sino proporcionar una evaluaci\'on factorial interpretable orientada a identificar la distribuci\'on estructural del error dentro del pipeline ITS.

\subsection{Implicaciones prácticas del F1 de instancia}
Aunque el mejor F1 de instancia obtenido (0.209) es bajo en términos absolutos, este resultado debe interpretarse dentro del contexto específico de ambientes ULS densos altamente complejos, donde la continuidad espacial de las copas, la estratificación vertical y el traslape geométrico generan condiciones extremadamente adversas para la separación individual de árboles.

Más que representar un fracaso aislado del pipeline evaluado, este desempeño evidencia una limitación práctica importante del ITS moderno bajo adquisiciones LiDAR de ultra alta densidad. La dificultad observada sugiere que, incluso bajo clasificación semántica casi perfecta, la separación geométrica de individuos continúa siendo un problema parcialmente abierto en bosques densos y estructuralmente continuos. En este sentido, el valor principal del presente trabajo no reside únicamente en el desempeño absoluto alcanzado, sino en evidenciar experimentalmente que el principal obstáculo para futuras mejoras ITS bajo ULS denso probablemente no se encuentra en la clasificación semántica, sino en los mecanismos de separación de instancias.
...
Estos resultados no implican que Watershed~3D represente el límite superior del desempeño posible, sino que evidencian cómo la continuidad geométrica de las copas puede limitar la separación de instancias incluso cuando la clasificación semántica es altamente precisa.

Más que proponer un nuevo método competitivo estado-del-arte, este trabajo aporta una disección experimental interpretable del pipeline ITS bajo ambientes ULS densos.

Trabajo futuro incluye comparación con TreeLearn~\cite{henrich2024} y SegmentAnyTree~\cite{wielgosz2024}, extensión a FOR-instanceV2~\cite{xiang2025}, y estrategias de seeding adaptativas que combinen CHM y densidad según la variabilidad local de altura.
%% ================================================================
%%  REFERENCIAS
%% ================================================================
