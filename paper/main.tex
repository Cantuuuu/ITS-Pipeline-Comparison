...
PN++: CHM $\rightarrow$ Density (C vs.\ E)           & $+0.061$ & $0.094$ & $0.066^{*}$  & $[+0.003, +0.110]$ & 9/11 \\
...
Con $n=11$ plots, incluso el test de permutaci\'on exacto --- que maximiza el poder estad\'istico al preservar la magnitud de las diferencias y calcular la distribuci\'on nula exacta ($2^{11}$ permutaciones) --- alcanza s\'olo significancia marginal para el efecto del density seeding ($p=0.096$ para RF, $p=0.066$ para PointNet++). Esto no invalida el hallazgo: el efecto es consistente en 8--9 de 11~plots y los intervalos BCa para PointNet++ excluyen el cero. Sin embargo, la validaci\'on en FOR-instanceV2~\cite{xiang2025}, que ofrece mayor n\'umero de plots y diversidad, es necesaria para confirmar la significancia estricta de este efecto. Las comparaciones entre preprocesadores s\'i alcanzan significancia plena (RF vs.\ PointNet++, $p=0.016$; baseline vs.\ RF+Density, $p=0.016$) y son robustas incluso con esta muestra.
...
PointNet++ se eval\'ua con una \'unica configuraci\'on de hiperpar\'ametros; una b\'usqueda exhaustiva podr\'ia mejorar su rendimiento. Por esta raz\'on, las comparaciones entre RF y PointNet++ deben interpretarse principalmente como observaciones dentro del pipeline experimental evaluado y no como una comparaci\'on definitiva entre paradigmas cl\'asicos y deep learning para segmentaci\'on forestal. Las conclusiones sobre el cuello de botella son espec\'ificas al Watershed~3D volum\'etrico; otros segmentadores (TreeLearn~\cite{henrich2024}, SegmentAnyTree~\cite{wielgosz2024}) podr\'ian mostrar una relaci\'on diferente. No obstante, los patrones de error observados --- especialmente la fusi\'on de copas adyacentes en canopias continuas --- sugieren una dificultad geom\'etrica m\'as amplia para la separaci\'on de instancias en ULS denso. Los resultados son espec\'ificos a datos ULS ($454$--$10\,156$~pts/m$^2$); las conclusiones presentadas deben interpretarse espec\'ificamente dentro del contexto de nubes ULS densas y no como una caracterizaci\'on universal del comportamiento de pipelines ITS sobre todas las modalidades LiDAR.

\subsection{Implicaciones pr\'acticas del F1 de instancia}
Aunque el mejor F1 de instancia obtenido (0.209) es bajo en t\'erminos absolutos, este resultado debe interpretarse dentro del contexto espec\'ifico de ambientes ULS densos altamente complejos, donde la continuidad espacial de las copas, la estratificaci\'on vertical y el traslape geom\'etrico generan condiciones extremadamente adversas para la separaci\'on individual de \'arboles.

M\'as que representar un fracaso aislado del pipeline evaluado, este desempe\~no evidencia una limitaci\'on pr\'actica importante del ITS moderno bajo adquisiciones LiDAR de ultra alta densidad. La dificultad observada sugiere que, incluso bajo clasificaci\'on sem\'antica casi perfecta, la separaci\'on geom\'etrica de individuos contin\'ua siendo un problema parcialmente abierto en bosques densos y estructuralmente continuos. En este sentido, el valor principal del presente trabajo no reside \'unicamente en el desempe\~no absoluto alcanzado, sino en evidenciar experimentalmente que el principal obst\'aculo para futuras mejoras ITS bajo ULS denso probablemente no se encuentra en la clasificaci\'on sem\'antica, sino en los mecanismos de separaci\'on de instancias.
...
La combinaci\'on RF + Watershed~3D Density alcanza el mejor F1 de instancia (0.209 en test), estableciendo una l\'inea base robusta y eficiente. Estos resultados no implican que Watershed~3D represente el l\'imite superior del desempe\~no posible, sino que evidencian c\'omo la continuidad geom\'etrica de las copas puede limitar la separaci\'on de instancias incluso cuando la clasificaci\'on sem\'antica es altamente precisa. La brecha entre calidad sem\'antica y calidad de instancia sugiere que las mejoras m\'as impactantes en ITS vendr\'an de segmentadores capaces de separar copas adyacentes en canopias densas --- mediante priors sobre geometr\'ia de copa, segmentadores end-to-end, o mejores estrategias de seeding. Trabajo futuro incluye comparaci\'on con TreeLearn~\cite{henrich2024} y SegmentAnyTree~\cite{wielgosz2024}, extensi\'on a FOR-instanceV2~\cite{xiang2025}, y estrategias de seeding adaptativas que combinen CHM y densidad seg\'un la variabilidad local de altura, as\'i como un an\'alisis detallado de la naturaleza del error (sobre- vs.\ infra-segmentaci\'on) por tipo de bosque y su relaci\'on con la geometr\'ia de copa. M\'as que proponer un nuevo m\'etodo competitivo estado-del-arte, este trabajo aporta una disecci\'on experimental interpretable del pipeline ITS bajo ambientes ULS densos. El c\'odigo fuente y los scripts de reproducci\'on est\'an disponibles en \url{https://github.com/Cantuuuu/ITS-Pipeline-Comparison}.
