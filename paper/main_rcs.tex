% main_rcs.tex — versión Computación y Sistemas (RCS), basada en main.tex
\documentclass{cys}
...
\addto\captionsenglish{\renewcommand{\figurename}{Fig.}\renewcommand{\abstractname}{Abstract.}}
\addto\captionsspanish{\renewcommand{\figurename}{Fig.}\renewcommand{\abstractname}{Resumen.}}
...
\title{La segmentación de instancias, no la semántica, limita la detección de árboles individuales en nubes de puntos ULS densas}

\author{Arturo Cantú Olivares$^1$, Diego Sebastian Cruz Cervantes$^1$, \\ Luis Fernando Maldonado$^1$, Jesús Gabriel Gudiño Lara$^1$, \\ Daniel Cantón Enriquez$^1$}

\affil{
$^1$ Universidad Autónoma de Querétaro (UAQ), Querétaro, \authorcr
México
\authorcr \authorcr
...
\othertitle{Instance Segmentation, Not Semantics, Limits Individual Tree Detection in Dense ULS Point Clouds}
...
\begin{keywords}
Individual Tree Segmentation, UAV laser scanning (ULS), Semantic segmentation, Watershed 3D, Density seeding, Random Forest, PointNet++, FOR-instance.
\end{keywords}

\selectlanguage{spanish}

\begin{abstract}
La segmentación automática de árboles individuales (ITS) en nubes de puntos de escaneo láser UAV (ULS) de alta densidad es crítica para inventarios forestales escalables; sin embargo, la contribución relativa del preprocesado semántico y la segmentación de instancias al error global del pipeline ITS rara vez se ha aislado experimentalmente. Presentamos una comparación factorial controlada ($3 \times 2$) de tres estrategias de preprocesado --- sin preprocesado, Random Forest (RF) con features geométricas y PointNet++ MSG --- combinadas con dos variantes de un segmentador Watershed 3D volumétrico interpretable (seeding CHM clásico y seeding por densidad), calibradas independientemente por flujo y evaluadas sobre el benchmark FOR-instance~V1 en cinco tipos de bosque. Pese a calidad semántica casi perfecta (tree~IoU~$>$~0.97 para ambos clasificadores), el F1 de instancia alcanza un máximo de 0.209, revelando una fuerte disociación entre calidad semántica y desempeño a nivel de instancia, donde la fuente dominante de error emerge de la separación de instancias y no del preprocesado semántico. Dentro de las configuraciones evaluadas, RF supera a PointNet++ MSG en todas las combinaciones sin requerir GPU, y el seeding por densidad mejora consistentemente sobre el seeding CHM (+47\% para RF, +74\% para PointNet++), particularmente en canopias densas de altura uniforme donde el CHM es casi plano. Estos hallazgos son específicos de adquisiciones ULS densas, donde la alta densidad de puntos y la continuidad de copas amplifican la complejidad geométrica de la separación individual.
\end{abstract}

\begin{keywords}
Segmentación de árboles individuales, escaneo láser UAV (ULS), segmentación semántica, Watershed 3D, seeding por densidad, Random Forest, PointNet++, FOR-instance.
\end{keywords}
