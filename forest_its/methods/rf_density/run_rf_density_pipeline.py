"""
Pipeline Flujo D: RF semántico (reutilizado) + Watershed 3D con seeding por densidad.

Este flujo reutiliza las predicciones semánticas del RF (Flujo B) ya guardadas
en output/predictions/rf/, pero aplica el watershed3d_density (seeding por
densidad de copa) en lugar del watershed3d estándar (seeding por CHM).

NO requiere re-entrenar ni re-correr el RF semántico. Solo necesita:
  1. Que existan las predicciones RF en output/predictions/rf/
  2. Que se haya corrido grid_search para rf_density (calibra top_band_m
     y demás params del watershed density)

Etapas:
  Stage 'instance'  — lee semantic_pred de RF, corre watershed_density con
                      los parámetros óptimos del grid search.
...
def _run_instance_single(
    ...
    """Lee semantic_pred de RF + corre watershed_density con best params."""
    plot_name = f"{las_path.parent.name}__{las_path.stem}"
    institution = las_path.parent.name

    # Reutilizar predicciones semánticas del RF
    ...
    # Filtrar puntos árbol (mismo filtro que RF)
    ...
    save_dir = output_dir / "predictions" / "rf_density"
...
def run_rf_density_pipeline(cfg, stage: str, split: str = "val"):
    """Ejecuta el pipeline RF+Density Watershed en la etapa indicada."""
    output_dir = Path(cfg.paths.output_dir)
    dataset_root = Path(cfg.paths.dataset_root)

    logger = _setup_logger(
        f"rf_density_pipeline_{stage}",
        output_dir / "logs" / f"rf_density_pipeline_{stage}_{split}.log",
    )

    plot_paths = _resolve_plot_paths(cfg, dataset_root, split)
    logger.info(f"=== RF Density Pipeline [{stage}] — {split} set — {len(plot_paths)} plots ===")

    if stage == "instance":
        # Cargar best params del grid search para rf_density
        try:
            ws_params = load_best_watershed_params("rf_density", cfg)
        ...
        all_metrics = []
        t_start = time.time()
        for las_path in tqdm(plot_paths, desc=f"RF density instance ({split})"):
            try:
        ...
        csv_path = results_dir / f"rf_density_metrics_{split}.csv"
    ...
    raise ValueError(f"Unknown stage: {stage}. rf_density only supports 'instance'.")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="RF (reused) + Watershed 3D Density Seeding pipeline"
    )
    ...
    run_rf_density_pipeline(cfg, stage=args.stage, split=args.split)
